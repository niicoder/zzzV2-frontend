...
export default (
  ...
  const canMaxWithdraw = userPoolData.staked > 0;
  // does user have rewards
  const canClaim = userPoolData.rewards.tokens > 0;
  // does user have anything to exit with
  const canExit = userPoolData.staked > 0 || userPoolData.rewards.tokens > 0;
   ...
   */
  const weeklyROI =
    basePoolData.APY && !poolClosed
      ? basePoolData.APY.weeklyROI * userPoolData.multiplier!
      : null;
